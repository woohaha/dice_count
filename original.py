...
while key_pressed != STOP_KEY:
    ...
    # create tmp images
    # 色size=orig depth=8 channel=1
    grey_scale = cv2.cv.CreateImage(cv2.cv.GetSize(orig), 8, 1)
    ...
    # Erode(src,dst,element=NULL,itertions=1)
    cv2.cv.Erode(grey_scale, processed, None, 10)
    ...
    # create a matrix 32bit depth, float, 3 channels
    storage = cv2.cv.CreateMat(orig.width, 1, cv2.cv.CV_32FC3)
    ...
    try:
        # extract circles
        cv2.cv.HoughCircles(
            processed, storage, cv2.cv.CV_HOUGH_GRADIENT, 2, 32.0, HIGH, LOW)
        ...
        for i in range(0, len(np.asarray(storage))):
            ...
            cv2.cv.Circle(
                orig, center, Radius, cv2.cv.CV_RGB(255, 0, 0), 3, 8, 0)

            cv2.cv.Circle(
                processed, center, 1, cv2.cv.CV_RGB(0, 255, 0), -1, 8, 0)
            cv2.cv.Circle(
                processed, center, Radius, cv2.cv.CV_RGB(255, 0, 0), 3, 8, 0)
